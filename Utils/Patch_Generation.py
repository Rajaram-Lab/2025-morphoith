...
def LoadPatchData(hdf5FileList,classDict=None,returnSampleNumbers=False,returnPatchCenters=False,showProgress=False):
...
def LoadSingleScalePatches(patchFiles, classDict):
    """
    Faster, shorter version of LoadPatchData. Use:
        trainPatchData, trainPatchClasses  = LoadSingleScalePatches(trainPatchList, classDict)
    """
    patchFiles = list(patchFiles) # collapse generators into list
    assert len(patchFiles) > 0, "empty patchFiles list provided"
    patches = []
    classes = []
    for patchFile in tqdm(patchFiles):
        with h5py.File(patchFile, mode='r') as f:
            patches.append(f['patches']['0'].value)
            classes.append(np.array([classDict[c.decode()] for c in f['classes']]))
    return np.concatenate(patches), np.concatenate(classes)
