...
import Utils.MoCoV2.builder as MoCoV2_builder
import Utils.Transforms as Transforms

# %%

parser = argparse.ArgumentParser(description='MoCov2 training')
parser.add_argument('--comp_type', default='same_patch',
                    choices=['same_patch', 'close_patch'],
...
class ImgDataSet(torch.utils.data.Dataset):
    def __init__(self, patches, compType, punchNumbers):
        self.patches = patches
        self.labels=punchNumbers
...
class DistributedProxySampler(torch.utils.data.DistributedSampler):
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    trainDataSet=ImgDataSet(trainingPatches,compType=args.comp_type,punchNumbers=trainingPunchNumbers)
    train_samplerPre=PairSampler(trainingLabels,args.batch_size)
    train_sampler=DistributedProxySampler(train_samplerPre)
    train_loader=torch.utils.data.DataLoader(trainDataSet,batch_sampler=train_sampler,num_workers=args.workers, pin_memory=True)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            }, is_best=False, filename=os.path.join(args.data, 'checkpoint_{:04d}.pth.tar'.format(epoch)))
